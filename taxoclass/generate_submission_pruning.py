...
def select_hierarchical_consistent(
    probs: np.ndarray,
    hierarchy,
    threshold: float = 0.5,
    min_labels: int = 2,
    ...
    """
    계층 구조를 강제로 유지(Closure)하며 개수 제약을 맞추는 로직
    전략:
    1. Threshold 넘는 노드 + 그 조상들 모두 선택 (Closure)
    2. 부족하면 확률 높은 순으로 추가 (조상 포함)
    3. 넘치면 '계층 구조를 해치지 않는 말단 노드(Leaf in set)'부터 확률 낮은 순 제거 (Bottom-up Pruning)
    """
    
    # 1. 초기 후보군 선정 (Threshold 이상)
    initial_candidates = set(np.where(probs >= threshold)[0].tolist())
    
    # 후보가 하나도 없으면 가장 확률 높은 것 하나 선택
    if not initial_candidates:
        initial_candidates.add(int(np.argmax(probs)))
    ...
    # 3. 최소 개수(min_labels) 맞추기
    # 부족하다면, 아직 선택되지 않은 노드 중 확률 높은 순으로 추가 (조상 포함)
    if len(selected) < min_labels:
        sorted_indices = np.argsort(probs)[::-1]
        for idx in sorted_indices:
            idx = int(idx)
            if idx not in selected:
                # 새로운 노드와 그 조상들을 임시 세트에 추가
                new_path = set([idx])
                new_path.update(hierarchy.get_ancestors(idx))
                
                # 추가
                selected.update(new_path)
                
                # 최소 개수를 채웠으면 중단
                if len(selected) >= min_labels:
                    break
    
    # 4. 최대 개수(max_labels) 맞추기 (핵심: Bottom-up Pruning)
    # 개수가 넘치면, "현재 선택된 집합 안에서 자식이 없는 노드"부터 제거해야 함
    # 그래야 부모가 자식보다 먼저 잘려나가는(Orphan 발생) 문제를 방지함
    
    while len(selected) > max_labels:
        # 현재 selected 집합 내에서의 Leaf 노드(자식이 선택되지 않은 노드) 찾기
        current_leaves = []
        for node in selected:
            # 이 노드의 자식들 중 하나라도 selected에 남아있다면, 이 노드는 제거 불가(부모이므로)
            children = hierarchy.get_children(node)
            is_parent_in_set = any(child in selected for child in children)
            
            if not is_parent_in_set:
                current_leaves.append(node)
        
        # 지울 수 있는 후보(Current Leaves) 중 확률이 가장 낮은 것 제거
        if current_leaves:
            victim = min(current_leaves, key=lambda x: probs[x])
            selected.remove(victim)
        else:
            # 이론상 DAG에서는 발생하지 않으나 안전장치: 단순 확률 최하위 제거
            victim = min(selected, key=lambda x: probs[x])
            selected.remove(victim)
...
def generate_submission(
    ...
    for i, pid in enumerate(tqdm(test_pids, desc="Formatting predictions")):
        ...
        predicted_classes = select_hierarchical_consistent(
            probs,
            hierarchy=hierarchy,
            threshold=threshold,
