...
from transformers import AutoTokenizer, AutoModel, DebertaV2Tokenizer, AutoConfig
...
# ============================================================================
def generate_backbone_class_embeddings(model_name, class_list, class_descriptions, device):
    """
...
class TELEClassTrainer:
    def __init__(self, num_classes, model_name, hidden_dim, class_embeddings, device_ids=None):
        ...
        self.model = ClassModel(model_name, hidden_dim, class_embeddings)
...
class CustomInference:
    def __init__(self, model_path, model_name, hidden_dim, class_embeddings, hierarchy_graph, device_ids=None):
...
class MultiGPUClassRepresentation:
    ...
    def create_enriched_class_descriptions(self, class_keywords, hierarchy_graph, idx_to_class):
        ...
        for class_name, keywords in tqdm(class_keywords.items(), desc="LLM Enrichment"):
            ...
            final_desc = f"{llm_desc} ; Keywords: {keyword_str}"
...
class MultiGPUTELEClassPipeline:
    ...
    def run(self):
        ...
        logger.info("\n" + "="*80)
        logger.info(f"PHASE 5: CUSTOM MODEL TRAINING DeBERTa")
        logger.info("="*80)
        ...
        primary_device = f'cuda:{self.device_ids[0]}' if self.device_ids else 'cpu'
        backbone_cls_embs = generate_backbone_class_embeddings(MODEL_NAME, data_loader.all_classes, class_descriptions, primary_device)
        ...
        trainer = TELEClassTrainer(
            ...
            class_embeddings=backbone_cls_embs, # [중요] BERT 임베딩 전달
        ...
        inference = CustomInference(
            ...
            model_name=MODEL_NAME,
            hidden_dim=HIDDEN_DIM,
            class_embeddings=backbone_cls_embs, # Inference 때도 구조 초기화를 위해 필요
            hierarchy_graph=data_loader.hierarchy_graph,
            device_ids=self.device_ids
        )
